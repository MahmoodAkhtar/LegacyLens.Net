...
### Step 8f: Code complexity artifact

Status: MVP scope addition

MVP scope:

- Add `code-complexity` as a supported optional artifact name accepted by explicit selection, comma-separated artifact selection, and `--artifacts all`.
- Generate `code-complexity.md` through the existing optional artifact runner model.
- Add a `CodeComplexityArtifactRunner` under `LegacyLens.Cli/Commands/Runners/` that consumes `ScanContext.FileInventory`, calls a core analyzer, writes the Markdown report, returns a `ScanArtifactResult`, and uses `ArtifactOutputPathResolver`.
- Update `ScanOptions`, `CliParser`, help text, console output, `ScanResult`, runner registration, and tests that enumerate supported artifacts.
- Add a `CodeComplexityAnalyzer` in `LegacyLens.Core.Analysis` using Roslyn syntax parsing over `ScanFileInventory.CSharpFiles`.
- Do not perform a new recursive filesystem scan, require solution build, load projects, restore packages, create a semantic model, compile, or execute code.
- Estimate member complexity from syntax by starting at `1` and incrementing for decision points such as `if`, `else if`, loops, switch sections, switch expression arms, `catch`, ternary conditionals, logical `&&` / `||`, and pattern `when` clauses where discoverable.
- Report supported members such as methods, constructors, local functions, property accessors, indexer accessors, operator overloads, conversion operators, and top-level statements where practical.
- Aggregate complexity by type, namespace, project, and scan root.
- Use simple severity bands such as Low 1-5, Moderate 6-10, High 11-20, and Very High 21+ as review heuristics.
- Flag likely generated files where cheaply detectable using evidence such as `<auto-generated>`, `.Designer.cs`, `.g.cs`, `.g.i.cs`, or `.generated.cs`.
- Add `CodeComplexityMarkdownReportWriter` with sections such as Summary, Highest Complexity Members, Highest Complexity Types, Project Summary, Namespace Summary, Type Summary, Member Details, and Notes and Limitations.
- Use shared Markdown-safe table-cell formatting for paths, signatures, evidence, member names, type names, and namespace names.
- Keep wording cautious: findings are static discovery signals for review, testing, simplification, and refactoring prioritisation.

Out of scope for MVP:

- Mermaid diagrams or large visual dependency graphs for code complexity.
- Exact Microsoft, Visual Studio, compiler, or build-aware code metrics.
- Runtime-risk, production-risk, defect-probability, testability, maintainability, correctness, or coverage claims.
- Automatic refactoring recommendations that imply a change is safe without developer review.
- Semantic-model, package-restore, project-load, or build-required analysis.
