...
The MVP artifact selection capability changes how optional reports are generated from the CLI. `--artifacts` should allow one artifact name, a comma-separated subset of artifact names, or `all`, while the normal `discovery-report.md` is always generated. `class-dependency-scope` is a parameterised artifact: it adds focused dependency analysis output only when selected with `--class-dependency-type <fully-qualified-type-name>`, or when `all` is used with that type option. `class-refactoring-opportunities` is also parameterised: it adds focused class refactoring-planning output only when selected with `--class-refactoring-type <fully-qualified-type-name>`, or when `all` is used with that type option. Plain `--artifacts all` should continue to generate normal batch artifacts without requiring scoped type options. Discovery and analysis scope remains defined by the specific selected artifacts. `code-complexity` is a normal optional artifact name and should be generated by explicit selection, comma-separated selection, or `all`.
...
- scoped class refactoring opportunities views for one requested fully qualified type, including evidence-backed testability barriers, existing seams, missing or weak seams, first characterization-test targets, relevant Working Effectively with Legacy Code-inspired techniques, and a cautious low-risk/high-value suggested order of approach
- static C# cyclomatic complexity estimates from indexed project-associated source files, including member-level results and type, namespace, project, and scan-root aggregates for `code-complexity.md`
- likely generated-code indicators for complexity results where cheaply detectable, such as `<auto-generated>`, `.Designer.cs`, `.g.cs`, `.g.i.cs`, or `.generated.cs`
- `.aspx` WebForms pages
...
- class-refactoring-opportunities inputs for timestamped scoped reports, using the shared project-aware C# file inventory and existing static outputs where practical to resolve a requested fully qualified class, inspect its source-visible shape, classify testability barriers, identify existing and missing seams, find public methods suitable for characterization tests, detect hardcoded construction, static/global access, configuration coupling, data-access or external dependency signals, method complexity hotspots, side-effect-like `void` methods, and dependency-breaking needs
- code-complexity inputs for `code-complexity.md`, using syntax-level evidence such as `if`, `else if`, loops, switch sections, switch expression arms, `catch`, ternary conditionals, logical `&&` / `||`, and pattern `when` clauses where discoverable to estimate member complexity and aggregate review hotspots
...
> Note: class-refactoring-opportunities is now MVP scope as a separate parameterised static report artifact. It should produce timestamped `class-refactoring-opportunities.<type>.<yyyyMMdd-HHmmss>.md` reports and should use static evidence to help a developer decide how to approach safely changing a requested class. It should be discriminating: suggest characterization tests, sensing, seams, dependency-breaking, sprout/wrap/extract techniques, extract interface, parameterize constructor, adapt parameter, or other Working Effectively with Legacy Code-inspired approaches only when supported by evidence. It should not refactor code, generate patches, build the solution, run tests, execute code, resolve runtime dependency injection, prove runtime call graphs, prove a dependency is unused, claim that refactoring is safe, or emit generic recommendations when evidence is weak.

> Note: code-complexity is now MVP scope as a separate static report artifact. It should produce `code-complexity.md` and should use indexed project-associated C# source files to estimate cyclomatic complexity from syntax without building the solution. It should report member, type, namespace, project, and scan-root summaries; flag likely generated files where cheaply detectable; and use severity bands such as Low, Moderate, High, and Very High as review heuristics. It should not claim to calculate exact Microsoft, Visual Studio, compiler, or build-aware code metrics; prove runtime behaviour, production risk, defect probability, testability, maintainability, or correctness; execute code; resolve runtime dependency injection; evaluate preprocessor symbols completely; or guarantee safe automatic refactoring.
