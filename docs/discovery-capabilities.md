...
The MVP artifact selection capability changes how optional reports are generated from the CLI. `--artifacts` should allow one artifact name, a comma-separated subset of artifact names, or `all`, while the normal `discovery-report.md` is always generated. `class-dependency-scope` is a parameterised artifact: it adds focused analysis output only when selected with `--class-dependency-type <fully-qualified-type-name>`, or when `all` is used with that type option. Plain `--artifacts all` should continue to generate normal batch artifacts without requiring a scoped type. Discovery and analysis scope remains defined by the specific selected artifacts.
...
## Markdown-Safe Report Formatting Note
...
- scoped source-level class/type dependency views for one requested fully qualified type, including direct outbound dependencies, direct inbound dependants, concerns involving the type, and a compact Mermaid diagram centred on that root type
- `.aspx` WebForms pages
...
- configuration-inventory analysis inputs for `configuration-inventory.md`, using static evidence such as configuration files, app settings, connection strings, custom configuration sections, environment transforms, WCF `system.serviceModel` sections, ASP.NET/IIS `system.web` and `system.webServer` sections, binding redirects, authentication and authorization settings, logging/diagnostics sections, Entity Framework configuration, SMTP/mail settings, configuration API usage such as `ConfigurationManager.AppSettings`, `ConfigurationManager.ConnectionStrings`, `IConfiguration`, and `GetSection` where discoverable, and source-code configuration key usage reconciliation for literal `ConfigurationManager.AppSettings[...]`, `ConfigurationManager.AppSettings.Get(...)`, and `ConfigurationManager.ConnectionStrings[...]` patterns where possible
...
- class-dependency-scope inputs for timestamped scoped reports, using the existing class dependency report to resolve a requested fully qualified root type and project direct inbound dependencies, direct outbound dependencies, related concerns, generated timestamp metadata, no-match or ambiguity information, and limitations where short-name or simple-name matching would be unsafe
- interface-inventory inputs for `interface-inventory.md`, using static C# source and visible configuration/XML evidence such as interface definitions, implementations, consumers, inherited interfaces, generic and collection-based interface usage, endpoint delegate parameters, service-locator calls, Microsoft DI registrations, legacy IoC registration patterns, Spring.NET object definitions, Castle Windsor XML, Unity XML, Enterprise Library/ObjectBuilder-style configuration, and custom object factory sections where discoverable; Spring.NET XML comments, `<description>` text, root `<objects>` text, and arbitrary descendant text are not registration evidence and should be ignored for matching and evidence snippets
...
> Note: configuration-inventory is now MVP scope as a separate static report artifact. It should produce `configuration-inventory.md` and should use static repository evidence to identify visible configuration files, sections, settings, transforms, source-code configuration access, and migration-relevant configuration concerns. It should map statically discoverable literal source-code configuration usages back to visible configured keys where possible, classify dynamic key usage as requiring review, and use cautious reconciliation wording such as `No static source usage detected` rather than `unused`. It should not claim to run the application, apply transforms, validate credentials, connect to external systems, prove production usage, prove settings are used or unused, fully evaluate runtime configuration inheritance, resolve deployment-time substitutions, expose secrets, or guarantee completeness. Sensitive values should be masked or redacted.
...
> Note: class-dependency-scope is now MVP scope as a parameterised static report artifact. It should produce timestamped files named like `class-dependency-scope.<safe-fully-qualified-type-name>.<yyyyMMdd-HHmmss>.md` when `--artifacts class-dependency-scope --class-dependency-type <fully-qualified-type-name>` is used. It should reuse the existing no-build class dependency analysis and shared file inventory, resolve the requested full type name case-insensitively, and report direct outbound dependencies, direct inbound dependants, concerns involving the root type, a compact Mermaid diagram, generated local and UTC timestamps, and clear no-match or ambiguity states. It should not claim runtime dependency injection resolution, reflection or dynamic loading understanding, transitive dependency completeness, generated-code behaviour, or runtime call graph accuracy.

> Note: interface-inventory is now MVP scope as a separate static report artifact. It should produce `interface-inventory.md` and should use static C# source and visible configuration/XML evidence to identify interface definitions, implementations, consumers, registration evidence, likely roles, possible extension points, multiple-implementation interfaces, missing static implementations, missing static consumers, and dynamic or configuration-driven wiring that requires review. It should inspect high-value Microsoft DI, Castle Windsor, Autofac, Ninject, Unity, StructureMap, Simple Injector, LightInject, Lamar, Common Service Locator, ASP.NET dependency resolver, Spring.NET XML, Castle Windsor XML, Unity XML, Enterprise Library/ObjectBuilder-style, and custom object factory evidence where feasible. Spring.NET XML evidence should be limited to meaningful object, property, constructor, alias, parent, abstract, and factory-style configuration; comments, descriptions, root container text, and broad serialized descendant text should not influence interface names, implementation names, findings, or evidence. It should not claim to build the solution, restore packages, execute code, load assemblies, apply transforms, resolve runtime dependency injection, prove runtime usage, prove an interface is unused, prove a registration is active, or guarantee completeness.
...
- detecting literal `ConfigurationManager.AppSettings["key"]`, `ConfigurationManager.AppSettings.Get("key")`, `ConfigurationManager.ConnectionStrings["name"]`, and similar fully qualified `System.Configuration.ConfigurationManager` access patterns where possible
- recording project name, source path, line number, concise evidence, usage kind, and literal key or name for statically discoverable source-code configuration usages
- classifying dynamic, computed, interpolated, variable-based, method-call-based, or concatenated configuration key access as requiring review without inventing a key
- reconciling literal source-used app setting keys and connection string names against visible configured entries using case-insensitive matching, including flattened JSON keys where feasible such as `ConnectionStrings:RabbitMQ`
- reporting configured keys with no detected source usage as `No static source usage detected`, without claiming that those keys are unused
...
## Interface Inventory Discovery

The `interface-inventory` capability should analyse C# source files and relevant visible XML/configuration files under discovered projects without building the solution. It should focus on abstractions and extension points rather than general class coupling.

Current MVP discovery expectations include:

- discovering C# interface declarations with name, full name where derivable, namespace, project, source path, line number, member counts, generic signature or arity, inherited interfaces, marker attributes such as `[ServiceContract]`, visibility where easy to identify, and concise source evidence
- discovering classes, records, and structs that implement interfaces through base lists, including generic interface evidence such as `IRepository<Customer>` while grouping cautiously by useful interface name
- discovering static consumers of interfaces through constructor parameters, fields, properties, method parameters, return types, local variables, generic type arguments, collection-based interface usage, inherited interfaces, endpoint delegate parameters, and service-locator or resolver calls where type arguments are visible
- detecting Microsoft DI registration evidence such as `AddSingleton`, `AddScoped`, `AddTransient`, `TryAdd*`, `typeof(IService), typeof(Service)`, and factory registrations that require review
- detecting legacy and third-party IoC registration evidence such as Castle Windsor, Autofac, Ninject, Unity, StructureMap, Simple Injector, LightInject, Lamar, Common Service Locator, ASP.NET MVC DependencyResolver, and Web API dependency resolver patterns where simple static syntax evidence is available
- detecting Spring.NET and similar XML/configuration-driven wiring evidence from `app.config`, `web.config`, `*.config`, external XML object-definition files, Castle Windsor XML, Unity XML, Enterprise Library/ObjectBuilder-style configuration, and custom object factory sections where feasible, while inspecting meaningful configuration-bearing elements and attributes rather than broad element text
- classifying factory-based, reflection-based, assembly-scanning, XML/configuration-driven, alias-based, parent/child-object, profile-based, and service-locator evidence as requiring review
- ignoring XML comments, Spring.NET `<description>` text, root `<objects>` evidence caused only by child comments or descriptions, and arbitrary descendant text when matching or reporting XML/configuration-driven interface registrations
- simplifying assembly-qualified XML type names from the type portion before the comma, so `SampleLegacyApp.Services.ICustomerService, SampleLegacyApp.Services` becomes `ICustomerService` and `SampleLegacyApp.Services.CustomerService, SampleLegacyApp.Services` becomes `CustomerService`
- identifying possible extension points, likely roles, interfaces with multiple implementations, interfaces with no static implementation found, interfaces with no static consumer found, and dynamic/configuration-driven wiring that may exist

Suggested report wording should stay cautious: `Static source evidence`, `Static configuration evidence`, `No static implementation found`, `No static consumer found`, `Registration evidence found`, `Dynamic wiring may exist`, `Configuration-driven wiring may exist`, `Requires review`, `Possible extension point`, `Likely role`, `Static analysis finding`, and `No static source usage detected`.
