...
| `ArtifactOutputPathResolver` | Centralises optional artifact output-path resolution for generated artifact files such as `upgrade-readiness-report.md`, `upgrade-blockers.md`, `external-dependencies.md`, `configuration-inventory.md`, `data-access-inventory.md`, `edmx-analysis.md`, `class-dependencies.md`, timestamped `class-dependency-scope.<type>.<timestamp>.md` reports, timestamped `class-refactoring-opportunities.<type>.<timestamp>.md` reports, `interface-inventory.md`, `solution-topology.md`, and `code-complexity.md`.                                                                      |
...
`class-dependency-scope` is a parameterised optional artifact. It should be a supported artifact name, but it requires `--class-dependency-type <fully-qualified-type-name>` when explicitly selected. `--artifacts all` should not require a type name and should not generate scoped dependency reports unless the type option is also supplied. Supplying `--class-dependency-type` without `class-dependency-scope` or `all` should be rejected during parsing.

`class-refactoring-opportunities` is also a parameterised optional artifact. It should be a supported artifact name, but it requires `--class-refactoring-type <fully-qualified-type-name>` when explicitly selected. `--artifacts all` should not require a class refactoring type and should not generate this scoped refactoring-planning report unless the type option is also supplied. Supplying `--class-refactoring-type` without `class-refactoring-opportunities` or `all` should be rejected during parsing. The existing `--class-dependency-type` option should remain unchanged for `class-dependency-scope`; do not repurpose it unless the CLI design is deliberately consolidated.
...
| `ClassRefactoringOpportunitiesArtifactRunner` | Produces timestamped `class-refactoring-opportunities.<type>.<yyyyMMdd-HHmmss>.md` files when `--artifacts class-refactoring-opportunities --class-refactoring-type <fully-qualified-type-name>` is selected, or when `all` is selected with a class refactoring type. It consumes `ScanContext`, reuses existing static analysis outputs where practical, and passes `ScanContext.FileInventory.CSharpFiles` into the analyzer for any class-source inspection. |
...
* For parameterised scoped class dependency output, build a safe filename from the requested fully qualified type name plus a local sortable `yyyyMMdd-HHmmss` timestamp before passing it to `ArtifactOutputPathResolver`, so repeated dependency review runs preserve historical reports.
* For parameterised class refactoring opportunities output, build a safe filename from the requested fully qualified type name plus a local sortable `yyyyMMdd-HHmmss` timestamp before passing it to `ArtifactOutputPathResolver`, and include both local and UTC generated timestamps in the report body.
* Use `Commands/Runners/` rather than `Commands/Artifacts/`, because `artifacts/` is a gitignored generated-output folder name.


#### Class Refactoring Opportunities Artifact Architecture

`class-refactoring-opportunities` should follow the existing optional artifact runner pattern. Add `ClassRefactoringOpportunitiesArtifactRunner` under `LegacyLens.Cli/Commands/Runners/` with `ArtifactName => "class-refactoring-opportunities"`. Its `ShouldRun` behaviour should be parameter-aware: explicit selection requires `ScanOptions.ClassRefactoringType`, and `--artifacts all` should generate this scoped report only when `ClassRefactoringType` is supplied. Use `ArtifactOutputPathResolver` with a filename of the form `class-refactoring-opportunities.<safe-fully-qualified-type-name>.<yyyyMMdd-HHmmss>.md`.

The runner should consume `ScanContext`, use `ScanContext.FileInventory`, and pass `ScanContext.FileInventory.CSharpFiles` or an equivalent `IReadOnlyCollection<ScanFile>` into `ClassRefactoringOpportunitiesAnalyzer`. The analyzer must not perform its own recursive filesystem walk such as `Directory.GetFiles(rootPath, "*.cs", SearchOption.AllDirectories)`. It should use `ScanFile.Content` where available and only fall back to defensive local reads of already-indexed paths if absolutely necessary.

The artifact should be an interpretation layer over existing static analysis, not a duplicate broad scanner. Prefer reusing class dependency analysis for type relationships, hardcoded `new`, static access, inbound/outbound context, and concerns; scoped class dependency behaviour for requested fully qualified type resolution; code complexity output for method/member hotspots; interface inventory output for existing seams; configuration inventory for configuration coupling; external dependencies for external system signals; and data-access analysis for repository, ORM, raw SQL, or database coupling signals. Artifact-specific Roslyn syntax inspection is acceptable only when it uses the shared C# file inventory and remains static/no-build.

Suggested core model types include `ClassRefactoringOpportunitiesAnalyzer`, `ClassRefactoringOpportunitiesReport`, `ClassRefactoringProfile`, `MethodRefactoringProfile`, `RefactoringSignal`, `TechniqueRecommendation`, `LegacyCodeTechnique`, `SuggestedRefactoringStep`, `ExistingSeam`, `MissingOrWeakSeam`, `TestabilityBarrier`, `CharacterizationTestTarget`, and `TestingPathClassification`. Prefer a signal-and-rule pipeline over one analyzer per Working Effectively with Legacy Code technique: class source plus existing reports, class refactoring profile, evidence-backed signals, recommendation rules, suggested approach, Markdown report.

The analyzer should remain no-build and cautious. It must not build the solution, restore packages, create semantic models requiring compilation, execute code, connect to external systems, resolve runtime dependency injection, prove runtime call graphs, prove a dependency is unused, automatically refactor code, or claim a refactoring is safe. Recommendations should be evidence-backed and discriminating; if there is not enough evidence, the report should say so rather than emitting generic advice.
