...
The MVP artifact selection capability changes how optional reports are generated from the CLI. `--artifacts` should allow one artifact name, a comma-separated subset of artifact names, or `all`, while the normal `discovery-report.md` is always generated. `class-dependency-scope` is a parameterised artifact: it adds focused analysis output only when selected with `--class-dependency-type <fully-qualified-type-name>`, or when `all` is used with that type option. Plain `--artifacts all` should continue to generate normal batch artifacts without requiring a scoped type. Discovery and analysis scope remains defined by the specific selected artifacts. `code-complexity` is a normal optional artifact name and should be generated by explicit selection, comma-separated selection, or `all`.
